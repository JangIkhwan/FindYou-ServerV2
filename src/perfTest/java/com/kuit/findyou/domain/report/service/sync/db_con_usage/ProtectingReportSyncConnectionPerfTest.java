...
class ProtectingReportSyncConnectionPerfTest {
    /*
    * 동기화 로직에서 외부 API 호훌 시에 DB 커넥션 점유 시간을 측정하는 클래스
    * */
    ...
    private final ProtectingReportSyncService syncService;
    ...
    @Autowired
    ProtectingReportSyncConnectionPerfTest(
    ...
    void measureConnectionHeldTimeByExternalApiLatency() throws Exception {
        ...
        String version = System.getProperty("syncConnPerfVersion", "before");
        Path outputPath = Path.of(System.getProperty(
                "syncConnPerfOutput",
                "build/reports/sync-connection-perf/results.csv"
        ));
    ...
    private void runMeasurement(String version, LatencyTarget target, int latencyMs,
        ...
        assertThat(finalCount).isEqualTo(NEW_REPORT_COUNT);

        System.out.printf(
                "target=%s, latency=%dms, run=%d, connectionHeld=%dms, acquireCount=%d, totalTime=%dms%n",
                target,
                latencyMs,
                run,
